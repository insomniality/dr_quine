...
#define STR "#include <stdlib.h>%1$c#include <string.h>%1$c#include <stdio.h>%1$c%1$c#define STR %2$c%3$s%2$c%1$c%1$cint main(void)%1$c{%1$c%5$cint i = %4$d;%1$c%5$cif (i <= 0) return (0);%1$c%5$cchar current_file[100];%1$c%5$csprintf(current_file, %2$cSully_%%d.c%2$c, i);%1$c%5$cif (!strcmp(current_file, __FILE__))i--;%1$c%5$cchar new_src_file[100];%1$c%5$csprintf(new_src_file, %2$cSully_%%d.c%2$c, i);%1$c%5$cchar new_exec[100];%1$c%5$csprintf(new_exec, %2$cSully_%%d%2$c, i);%1$c%5$cFILE *f = fopen(new_src_file, %2$cw%2$c);%1$c%5$cif (f == NULL) return (1);%1$c%5$cfprintf(f, STR, 10, 34, STR, i, 9);%1$c%5$cfclose(f);%1$c%5$cchar compile_cmd[300];%1$c%5$csprintf(compile_cmd, %2$cgcc -o %%s %%s%2$c, new_exec, new_src_file);%1$c%5$csystem(compile_cmd);%1$c%5$cchar run_cmd[200];%1$c%5$csprintf(run_cmd, %2$c./%%s%2$c, new_exec);%1$c%5$csystem(run_cmd);%1$c}%1$c"
